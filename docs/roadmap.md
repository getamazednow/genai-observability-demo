...
| **Weeks 7–10: Govern** | Operationalise responsible AI and security controls | Integrate Sensitive Data Scanner/DLP, prompt-injection signals, custom evaluations, human-review telemetry, audit evidence, high-risk tool approval metrics, **first-class decision records** | High-risk workflows have policy-decision telemetry, audit evidence, approval/override tracking, responsible AI dashboards, **inspectable decision traces** | 🟢 **Largely done in mock form.** Guardrail/policy-decision telemetry, approval-bypass tracking, a mocked evaluation-harness series, **and now first-class decision tracing** (`docs/decision-contract.md`: decision records with evidence, options, selection basis, policy result, authority and business outcome; Decision Trace dashboard tab; explainability/authority coverage metrics; decision-override monitor) are all modelled. **Still not modelled:** a *real* eval harness (LLM-as-judge, human review sampling), bias/fairness flags, refusal quality, and cross-version decision replay/comparison (decision-contract.md P2) — see `docs/metrics-catalogue.md` for the exact remaining gap list |
