...
create_issue \
  "[Phase 0] Preflight: owners, risk tier, redaction policy, Datadog org provisioning" \
  "phase-0-preflight,human-gate" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 1] Instrumentation scaffold: replace synthetic generator with real spans" \
  "phase-1-instrumentation,agent-executable" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 2] Ingestion wiring: point instrumentation at a staging Datadog org" \
  "phase-2-ingestion,agent-executable" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 3] Import dashboards, monitors and SLOs into staging Datadog" \
  "phase-3-dashboards,agent-executable" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 3b] Re-point monitor alerts to real on-call rotation" \
  "phase-3-dashboards,human-gate" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 4] Real evaluation harness: replace synthetic eval series" \
  "phase-4-governance,human-gate" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 4] Model the remaining governance gaps: bias/fairness, refusal quality, override rate" \
  "phase-4-governance" \
  "$(cat <<'BODY'
...
create_issue \
  "[Phase 5] Extract into a reusable instrumentation library and onboarding checklist" \
  "phase-5-scale,agent-executable" \
  "$(cat <<'BODY'
...
create_issue \
  "[Repo hygiene] Branch protection + required PR review on main" \
  "repo-hygiene" \
  "$(cat <<'BODY'
...
create_issue \
  "[Repo hygiene] Add CONTRIBUTING.md and issue/PR templates" \
  "repo-hygiene" \
  "$(cat <<'BODY'
...
echo "Done."
if [[ -n "$PROJECT_NUMBER" ]]; then
  echo "Board: ${PROJECT_URL:-<see gh project list --owner $OWNER>}"
else
